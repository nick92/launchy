...
public class Launcher.Widgets.AppEntry : Gtk.Button {
    ...
    public AppEntry (Backend.App app) {
        ...
        exec_name = app.exec;
        icon_size = Launchy.settings.icon_size;
        icon = app.icon;
        ...
        if (Launchy.settings.font_size <= 0.001) {
            ...
            var texto = "<span font_size=\"%d\">%s</span>".printf((int)(Launchy.settings.font_size * 1000),app_name);
        ...
        this.drag_end.connect ( () => {
            ...
            ((LaunchyView)panther_app.active_window).grab_device ();
        });
  ...
  private Gtk.MenuItem get_saved_menuitem () {
      //docked = (desktop_uri in plank_client.get_persistent_applications ());
      var panther_app = (Gtk.Application) GLib.Application.get_default ();
      bool saved = ((LaunchyView)panther_app.active_window).cat_saved;
    ...
    if (is_valid) {
      ...
      try {
        ...
        if (!dockitem_file.query_exists ()) {
          ...
          var panther_app = (Gtk.Application) GLib.Application.get_default ();
          ((LaunchyView)panther_app.active_window).amend_stared (basename);
        }
      } catch (Error e){
        warning(e.message);
      }
